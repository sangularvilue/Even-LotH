...
export async function startHeadGestures(bridge: EvenAppBridge, cb: GestureCallback): Promise<boolean> {
  callback = cb
  sampleCount = 0
  lastGestureTime = 0
  ...
  try {
    ...
    if (result === false || result === 'error') {
      console.log('[HeadGestures] IMU not available')
      return false
    }

    // Listen for IMU data events
    // The event format isn't typed yet — try common patterns
    bridge.onEvenHubEvent((event: any) => {
      // IMU data might come as a custom event type
      const imuData = event.imuEvent ?? event.sensorEvent ?? event.motionEvent
      if (imuData) {
        const x = imuData.x ?? imuData.accX ?? 0
        const y = imuData.y ?? imuData.accY ?? 0
        const z = imuData.z ?? imuData.accZ ?? 0
